...
def get_auth_url():
    ...
    params = {
        ...
        "state": frappe.generate_hash(length=10) # Add CSRF protection
    }
    # Store state in cache for verification later
    frappe.cache().hset("xero_oauth_state", frappe.session.user, params["state"])
...
def handle_oauth_callback(code=None, state=None, error=None):
    ...
    # Verify state against cached value
    cached_state = frappe.cache().hget("xero_oauth_state", frappe.session.user)
    ...
    try:
        ...
        settings.token_expiry = add_to_date(now_datetime(), seconds=token_data["expires_in"])
        settings.tenant_id = tenant_id
        settings.save(ignore_permissions=True)
        frappe.db.commit()

        frappe.local.response["type"] = "redirect"
        frappe.local.response["location"] = "/app/xero-settings" # Redirect back to settings page
        ...
        frappe.log_error(message=frappe.get_traceback(), title="Xero OAuth Callback Error")
...
def refresh_access_token():
    ...
    while retry_count < max_retries:
        try:
            ...
            settings.access_token = token_data["access_token"]
            settings.refresh_token = token_data["refresh_token"] # Xero might issue a new refresh token
            settings.token_expiry = add_to_date(now_datetime(), seconds=token_data["expires_in"])
            ...
            if hasattr(e, 'response') and e.response is not None:
                if e.response.status_code in [400, 401]:
                    ...
                    frappe.throw("Xero refresh token is invalid. Please re-authenticate in Xero Settings.")
            ...
            if retry_count >= max_retries:
                ...
                notify_admins_token_failure(f"Token refresh failed after {max_retries} attempts")
                frappe.throw("Failed to refresh Xero access token after multiple attempts. Please check Xero Settings.")
...
def notify_admins_token_failure(reason):
    ...
    try:
        ...
        frappe.log_error(f"Failed to notify admins about token failure: {str(e)}", "Xero Notification Error")
...
def get_xero_client():
    ...
    if not settings.access_token or not settings.tenant_id:
        frappe.throw("Xero connection not configured or access token missing. Please connect in Xero Settings.")

    # Check if token is expired or close to expiring (e.g., within 5 minutes)
    from frappe.utils import get_datetime
    token_expiry_dt = get_datetime(settings.token_expiry) if settings.token_expiry else None

    if not token_expiry_dt or now_datetime() >= add_to_date(token_expiry_dt, minutes=-5):
    ...
    while retry_count < max_retries:
        try:
            if method.upper() == "GET":
                ...
                response = requests.post(url, headers=headers, data=dumps(data) if data else None, params=params, timeout=api_timeout)
            elif method.upper() == "PUT":
                response = requests.put(url, headers=headers, data=dumps(data) if data else None, params=params, timeout=api_timeout)
            else:
            ...
            if e.response.status_code == 429:  # Rate limit error
                ...
                rate_limit_remaining = e.response.headers.get('X-Rate-Limit-Remaining', 'unknown')
                ...
                if retry_count >= max_retries:
                    ...
                    frappe.log_error(f"Xero API rate limit exceeded after {max_retries} retries. Remaining: {rate_limit_remaining}", "Xero API Error")
                    raise e
                
                # Exponential backoff with configurable base and max delay
                wait_time = min(backoff_base ** retry_count, max_delay)
                frappe.log_warning(f"Xero rate limit hit. Retrying in {wait_time} seconds... (Attempt {retry_count}/{max_retries})", "Xero API Error")
                time.sleep(wait_time)
            else:
            ...
            frappe.log_error(message=f"Xero Network Error on {method} {url}: {e}", title="Xero Network Error")
            frappe.throw(f"Network error communicating with Xero: {e}")
        
        except Exception as e:
            frappe.log_error(message=frappe.get_traceback(), title="Xero Client Unexpected Error")
...
def check_xero_entity_exists(entity_type, entity_id):
    ...
    try:
        ...
        frappe.log_error(f"Error checking if {entity_type} {entity_id} exists: {str(e)}", "Xero Entity Check")
...
def get_xero_entity_by_number(entity_type, number_field, number_value):
    ...
    try:
        ...
        response = xero_request("GET", entity_type, params={
            "where": f'{number_field}=="{number_value}"'
        })
        ...
        frappe.log_error(f"Error searching {entity_type} by {number_field}={number_value}: {str(e)}", "Xero Search Error")
        return None
