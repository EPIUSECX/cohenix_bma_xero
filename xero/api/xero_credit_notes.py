...
def sync_return_to_xero(doc_name, doc_type, **kwargs):
    ...
    try:
        ...
        for item in doc.items:
            ...
            if item.item_code:
                xero_item_id = frappe.db.get_value("Item", item.item_code, "xero_item_id")
                if xero_item_id:
        ...
        for tax in doc.taxes:
            ...
            tax_description = validate_credit_note_description(tax.description, "Tax/Charge")
        ...
        if cn_type == "ACCRECCREDIT":
            reference = validate_credit_note_reference(doc.get("remarks") or doc.get("po_no"))
            if reference:
        ...
        response = xero_request("POST", "CreditNotes", data={"CreditNotes": [cn_payload]})
...
def enqueue_void_credit_note(doc, method):
    ...
    frappe.logger().info(f"Queued void for {doc.doctype} {doc.name} in Xero.", "Xero Sync")
...
def void_credit_note_in_xero(doc_name, doc_type):
    ...
    try:
        ...
        if response and response.get("CreditNotes"):
            frappe.db.set_value(
                doc_type, doc_name,
                "xero_sync_status", f"{action.capitalize()}d in Xero",
                update_modified=False
            )
            frappe.db.commit()
            log_xero_error(
            ...
            raise Exception("Invalid response received from Xero when voiding/deleting credit note.")
...
def process_xero_credit_note(xero_cn_data, settings):
    ...
    try:
        ...
        remarks_parts = [f"Xero Credit Note: {cn_number}" if cn_number else "Xero Credit Note"]
        ...
        if erpnext_doc_name:
            ...
            for line in line_items:
                ...
                item_dict = {
                    ...
                    "amount": -abs(flt(line.get("LineAmount", 0))),  # Negative for return
                }
